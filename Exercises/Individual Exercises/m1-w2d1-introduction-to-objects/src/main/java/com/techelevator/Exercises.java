...
public class Exercises {
	 ...
	 * "Hello Bob!". helloName("Bob") → "Hello Bob!" helloName("Alice") →
	 * "Hello Alice!" helloName("X") → "Hello X!"
	 */
	 ...
	 * the order abba, e.g. "Hi" and "Bye" returns "HiByeByeHi". makeAbba("Hi",
	 * "Bye") → "HiByeByeHi" makeAbba("Yo", "Alice") → "YoAliceAliceYo"
	 ...
	 * string with tags around the word, e.g. "<i>Yay</i>". makeTags("i", "Yay")
	 * → "<i>Yay</i>" makeTags("i", "Hello") → "<i>Hello</i>" makeTags("cite",
	 * "Yay") → "<cite>Yay</cite>"
	 */
	 ...
	 * at index i and going up to but not including index j. makeOutWord("<<>>",
	 * "Yay") → "<<Yay>>" makeOutWord("<<>>", "WooHoo") → "<<WooHoo>>"
	...
	public String extraEnd(String str) {
		String repeatLast2Chars = "";
		String last2Chars = str.substring(str.length() - 2);

		for (int i = 0; i < 3; i++) {
			repeatLast2Chars += last2Chars;
		}

		return repeatLast2Chars;
	}
	 ...
	 * a string. firstTwo("Hello") → "He" firstTwo("abcdefg") → "ab"
	 * firstTwo("ab") → "ab"
	 */
	 ...
	 * "WooHoo" yields "Woo". firstHalf("WooHoo") → "Woo"
	...
	public String withoutEnd(String str) {
		...
		String strMinusFirstLastChar = strMinusFirstChar.substring(0, strMinusFirstChar.length() - 1);

		return strMinusFirstLastChar;
	}
	 ...
	 * (length 0). comboString("Hello", "hi") → "hiHellohi" comboString("hi",
	 * "Hello") → "hiHellohi" comboString("aaa", "b") → "baaab"
	 */
	 ...
	 * of each. The strings will be at least length 1. nonStart("Hello",
	 * "There") → "ellohere" nonStart("java", "code") → "avaode"
	 * nonStart("shotl", "java") → "hotlava"
	 */
	public String nonStart(String a, String b) {
		String aMinusFirstChar = a.substring(1);
		String bMinusFirstChar = b.substring(1);

		return aMinusFirstChar + bMinusFirstChar;
	}
	...
	public String right2(String str) {
		...
		String charsBeforeLast2 = str.substring(0, str.length() - 2);
		String last2Chars = str.substring(str.length() - 2);

		return last2Chars + charsBeforeLast2;
	}
	 ...
	 * will be non-empty. theEnd("Hello", true) → "H" theEnd("Hello", false) →
	 * "o" theEnd("oh", true) → "o"
	 */
	...
	public String withouEnd2(String str) {
		...
		String strMinusFirstChar = str.substring(1);
		String strMinusFirstAndLastChar = strMinusFirstChar.substring(0, strMinusFirstChar.length() - 1);

		return strMinusFirstAndLastChar;
	}
	 ...
	 * least 2. middleTwo("string") → "ri" middleTwo("code") → "od"
	...
	public String middleTwo(String str) {
		...
		if (str.length() % 2 == 0) {
			middleSubstring = str.substring(middle - 1, middle + 1);
		}
	...
	public String middleThree(String str) {
		...
		if (str.length() % 2 != 0) {
			middleSubstring = str.substring(middle - 1, middle + 2);
		}
	 ...
	 * strings. hasBad("badxx") → true hasBad("xbadxx") → true hasBad("xxbadxx")
	 * → false
	 */
	public boolean hasBad(String str) {
		boolean badIsIndex0Or1 = str.indexOf("bad") == 0 || str.indexOf("bad") == 1;
	 ...
	 * than length 3. Return n copies of the front; frontTimes("Chocolate", 2) →
	 * "ChoCho" frontTimes("Chocolate", 3) → "ChoChoCho" frontTimes("Abc", 3) →
	 * "AbcAbcAbc"
	 */
	public String frontTimes(String str, int n) {
		String frontOfStr = "";
		String frontOfStrRepeated = "";

		if (str.length() < 3) {
			frontOfStr += str;
		} else {
			frontOfStr += str.substring(0, 3);
		}

		for (int i = 0; i < n; i++) {
			frontOfStrRepeated += frontOfStr;
		}

		return frontOfStrRepeated;
	}

	/*
	 * Count the number of "xx" in the given string. We'll say that overlapping
	 * is allowed, so "xxx" contains 2 "xx". countXX("abcxx") → 1 countXX("xxx")
	 * → 2 countXX("xxxx") →
	 */
	public int countXX(String str) {
		...
		for (int i = 1; i < str.length(); i++) {
			//if the character and the character to its left equal x
	...
	public boolean doubleX(String str) {
		...
		if (indexOfFirstX != -1) {
			//if there are 2 chars left to check and the substring equals "xx"
			if ((indexOfFirstX + 2) < str.length() && str.substring(indexOfFirstX, indexOfFirstX + 2).equals("xx")) {
				return true;
			}
		}
	 ...
	 * with the first, so "Hello" yields "Hlo". stringBits("Hello") → "Hlo"
	 * stringBits("Hi") → "H" stringBits("Heeololeo") → "Hello"
	 */
	public String stringBits(String str) {
		String strBitsEveryOther = "";

		for (int i = 0; i < str.length(); i += 2) {
			strBitsEveryOther += str.substring(i, i + 1);
		}

		return strBitsEveryOther;
	}
	...
	public String stringX(String str) {
		...
		String newString = "";
		String strRemoveXs = str.replaceAll("x", "");
		...
		if (str.charAt(str.length() - 1) == 'x') {
			newString += strRemoveXs + 'x';
		} else {
			newString += strRemoveXs;
		}
	 ...
	 * 8,9 ... so "kittens" yields "kien". altPairs("kitten") → "kien"
	...
	public String altPairs(String str) {
		...
		for (int i = 0; i < str.length(); i += 4) {
			// check if str has only one char left
			if (str.length() > i + 2) {
				altPairsString += str.substring(i, i + 2);
			} else {
				altPairsString += str.substring(i);
			}
		}
	 ...
	 * strings will not overlap. stringYak("yakpak") → "pak" stringYak("pakyak")
	 * → "pak" stringYak("yak123ya") → "123ya"
	 */
